...
各区間の割線勾配を (\delta_j) とし、共通端点勾配を

\[
m=\min\!\left(G,\;0.95\min_j\delta_j\right)>0
\]

として

\[
s'(0)=s'(1/2)=s'(1)=s'(3/2)=m,
\qquad s''(0)=s''(1/2)=s''(1)=s''(3/2)=0
\]

とする。等しい端点勾配を持つ零曲率quintic区間の導関数は

\[
s'=m+30t^2(1-t)^2(\delta_j-m)
\]

なので、全方向で (m\le\delta_j) とすることにより (s'>0) が保証される。これにより、
PMMAモスアイ先端のように物理内外半径比が固定計算support比から大きく外れる場合でも、
中心の単位勾配によるovershootとJacobianの符号反転を防ぐ。

物理座標は (\mathbf q'=s\mathbf q_I) である。値・一次微分・二次微分を共有するため、
半径方向にはC2である。周期境界では (s=\rho_b) により写像値が恒等写像へ戻り、反対側も
同じ正勾配 (m) を持つ。直交格子のD4または三角格子のD6 sector境界ではsupportの `max`
に由来して区分的に滑らかである。
