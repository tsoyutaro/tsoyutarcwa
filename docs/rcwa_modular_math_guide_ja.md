...
### 5.5 コアシェルの二重matched半径写像

`radial_mapping="double"` では、直交格子の矩形supportまたは三角格子の六角形supportを
(h(\mathbf q))、その周期セル境界値を (H) とする。計算空間の界面を物理半径に依存させず、

\[
h_c=H/3,\qquad h_o=2H/3,\qquad
\rho=\frac{h}{h_o}
\]

と固定する。したがって内界面、外界面、周期境界はそれぞれ
(\rho_c=1/2)、(\rho_o=1)、(\rho_b=3/2) である。同じray上で

\[
\mathbf q_I=\frac{\mathbf q}{\rho},\qquad
c_c=\frac{R_c}{\|\mathbf q_I\|},\qquad
c_o=\frac{R_o}{\|\mathbf q_I\|}
\]

とし、半径profile (s(\rho)) を3本のquintic Hermite区間で構成する。

\[
s(0)=0,\quad s(1/2)=c_c,\quad s(1)=c_o,\quad s(3/2)=3/2,
\]

各区間の最小方向割線勾配を (\delta_c,\delta_s,\delta_b) とする。物質界面だけに

\[
m_I=\min\!\left(G,\;0.95\min_j\delta_j\right)>0
\]

を与え、中心と周期境界には

\[
m_0=\min(1,0.95\delta_c),\qquad
m_b=\min(1,0.95\delta_b)
\]

を使う。したがって端点勾配は

\[
s'(0)=m_0,\quad s'(1/2)=s'(1)=m_I,\quad s'(3/2)=m_b
\]

であり、各節点の二階微分は零とする。割線 (\delta) と端点勾配 (m_0,m_1) を持つ
零曲率quinticの半径導関数をBernstein基底で表すと、その係数は

\[
\{m_0,m_0,5\delta-2m_0-2m_1,m_1,m_1\}
\]

である。両端勾配を正かつ (\delta) 以下に制限したため全係数が正で、全方向で
(s'>0) が保証される。これにより中心の単位勾配によるovershootを防ぐ一方、界面用の
小さい (G) を中心・周期境界へ強制して (\det J\sim G^2) とする過圧縮も避ける。

物理座標は (\mathbf q'=s\mathbf q_I) である。値・一次微分・二次微分を共有するため、
半径方向にはC2である。周期境界では (s=\rho_b) により写像値が恒等写像へ戻り、反対側も
同じ正勾配 (m_b) を持つ。直交格子のD4または三角格子のD6 sector境界ではsupportの `max`
に由来して区分的に滑らかである。

物質maskは計算空間の固定条件 (\rho\le1/2)、(\rho\le1) から作る。したがって不連続maskを
半径変更でsampling点間へ移動させず、(R_c,R_o) は (c_c,c_o) とJacobianだけを通って
微分される。実装検証では両半径のautogradが中心差分と一致する。

既存の逐次u/v対称factorizationは、一つのWeiss座標線族を前提とするため、そのまま
二重放射supportへ流用しない。代わりに、内外界面に共通するlevel set
`rho`から法線共変量
...
を作り、一般化Li因数分解を適用する。未変形計算格子の計量を

\[
g_{ij}=\begin{pmatrix}1&\cos\zeta\\\cos\zeta&1\end{pmatrix},
\qquad n^i=g^{ij}n_j,
\]

とし、正規化したmixed projectorを

\[
(P_n)_i{}^j=\frac{n_i n^j}{n_kn^k},\qquad P_t=I-P_n
\]

とする。変換媒質tensorを (D^i=A^{ij}E_j)、(C_i{}^j=g_{ik}A^{kj}) と書けば、

\[
B=P_t+P_nC,\qquad G=BE=P_tE+P_nD^\flat
\]

である。界面上では (G) が連続な接線電場と法線電束から成るため、Liの逆則を
(E=B^{-1}G) に適用して

\[
C^{F}=[CB^{-1}]\,[B^{-1}]^{-1},\qquad A^{F}=g^{-1}C^{F}
\]

を得る。ここで角括弧は2次元Toeplitz畳み込み行列である。直交格子、scalar
(A=\varepsilon I)、u法線では

\[
A^F=\operatorname{diag}\!\left([1/\varepsilon]^{-1},[\varepsilon]\right)
\]
...
corner harmonicを介したD6対称性の破れを持ち込まない。
...
```bash
python validation/validate_design_gradients.py
...
python validation/validate_rcwa_solver_auto.py --order 1 --grid 64
```
