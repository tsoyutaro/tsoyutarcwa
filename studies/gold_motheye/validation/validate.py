...
from studies.shared.gold_dispersion import gold_epsilon_rakic_ld, load_gold_csv
...
def core_checks() -> list[dict[str, object]]:
    ...
    convergence_source = (_PACKAGE_ROOT / "converge.py").read_text(
        encoding="utf-8"
    )
    checks.append(
        {
...
def main() -> int:
    ...
    parser.add_argument(
        "--json",
        type=Path,
        default=_PACKAGE_ROOT / "validation" / "results" / "setup_validation.json",
    )
