...
`studies/gold_motheye/converge.py` は、まだ指定されていない形状条件を次の既定値で補っている。
...
```bash
python studies/gold_motheye/converge.py --symmetry-reduction cs-source --device cuda
```

測定CSVを使う例:

```bash
python studies/gold_motheye/converge.py \
  --gold-model csv --gold-csv au_measured_nk.csv \
  --device cuda --output-prefix studies/gold_motheye/results/au_motheye_measured
```
...
```bash
python studies/gold_motheye/validation/validate.py
python studies/gold_motheye/validation/validate.py --integration --device cuda
```
