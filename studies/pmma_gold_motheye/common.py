...
def simulate_case(
    ...
    *,
    ...
    factorization_schemes = sorted(
        {
            str(record.options["factorization_scheme"])
...
def add_shared_arguments(parser) -> None:
    ...
    parser.add_argument(
        ...
        default="outer",
        help="Core-shell slices: match the outer circle only or both radii.",
    )
...
def run_axis_convergence(
    *,
    ...
    assumptions = {
        ...
        "asr_statement": (
            "both concentric interfaces use the C2 radial matched map and "
            "generalized Li normal-D/tangential-E factorization"
            if args.radial_mapping == "double"
            else "outer Au/air boundary matched exactly; concentric Au/PMMA "
