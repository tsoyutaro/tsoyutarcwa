...
再現コードは `reproduce_asr_fig8.py`、独立検証は `validate_asr_fig8.py` にある。
...
出力先は既定で `asr_fig8_results/` であり、以下を生成する。
...
python validate_asr_fig8.py --integration
```

Redhefferとの独立比較には次を使える。

```bash
python reproduce_asr_fig8.py --study smoke --cascade algo2a --output-dir smoke_algo2a
```

6 GHzで図9型の次数収束も実行できる。

```bash
python reproduce_asr_fig8.py --study convergence --max-order 24 --device cuda --no-plot
```
...
```bash
python reproduce_asr_fig8.py --study fig8 --resume --hfss-csv hfss_fig8.csv
```
