...
現在の完全D6経路は各blockで固有modeを求めた後、全modeをnative-star基底へ再結合して
...
xまたはyの一つのsourceだけを求める既存Cs経路では固有値次元とcascade次元が概ね
\(D_\star/2\) となる。密固有値主要項だけならstar一括比約8倍だが、完全D6はより強い
12要素対称性とmatrix unitを使うため全modeを求めても約86.4倍になる。一方、完全D6のcascadeは
全star次元へ戻り、Cs cascadeは半次元のままである。したがって、層固有値計算が支配的なら
完全D6、単一sourceの多層cascadeが支配的ならCsが有利になり得る。
...
D6/Csによる固有値問題の短縮とnative-star/Cs次元でのcascade短縮はそのまま有効である。
...
前後方向内部field coupling再構成を含む14項目すべてに合格した。
...
前後方向coupling、接線場の層界面連続性、空間場合成、fields-only公開S無効化を含めている。
