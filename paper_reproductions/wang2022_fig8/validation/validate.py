...
def _static_checks(package_root: Path, outputs_root: Path) -> list[Check]:
    checks: list[Check] = []
    reproduction = package_root / "reproduce.py"
    asr_source_path = outputs_root / "rcwa_ext" / "asr.py"
...
def _run_smoke(package_root: Path, output: Path, cascade: str, methods: str) -> list[dict[str, object]]:
    command = [
        sys.executable,
        str(package_root / "reproduce.py"),
        "--study",
        "smoke",
        "--methods",
        methods,
...
def _paper_stress_checks(package_root: Path) -> list[Check]:
    checks: list[Check] = []
    missing = [
        module
        for module in ("torch", "torcwa")
...
def main(args: argparse.Namespace) -> dict[str, object]:
    checks = _static_checks(_PACKAGE_ROOT, _OUTPUTS_ROOT)
    if args.integration:
        checks.extend(_integration_checks(_PACKAGE_ROOT))
    if args.paper_stress:
        checks.extend(_paper_stress_checks(_PACKAGE_ROOT))
